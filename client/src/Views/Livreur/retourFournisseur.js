...
function RetourAuFournisseurColis() {
        ...
        if (isScanning) {
            try {
                ...
                if (match) {
                    ...
                    axios
                        .post('http://localhost:3000/retourAuFournisseur', {
                            id: parsedData.id,
                        }, {
                            headers: {
                        ...
                        .then((response) => {
                            console.log('Colis retour Au Fournisseur:', response.data);
                            // Show a success snackbar when a colis is added to stock
                            stopScanning();
                            enqueueSnackbar('Colis retour Au Fournisseur', {variant: 'success'});
                        })
                        .catch((error) => {
                            if (error.response && error.response.data && error.response.data.error) {
                                ...
                                enqueueSnackbar('Erreur retour Au Fournisseur ', {variant: 'error'});
                            }
                        });
                }
            } catch (error) {
        ...
        <>
            ...
            <SnackbarProvider maxSnack={3}>
                <div>
                    ...
                    >
                        {!isScanning ? (
                            <Button onClick={startScanning}>Start Scanning Colis Retour Au Fournisseur </Button>
                        ) : (
...
export default RetourAuFournisseurColis;